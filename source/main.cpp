...
#define STBTT_STATIC  // Make the implementation private to the file that generates the STB True Type implementation.

/**
 * Init Tesla STB True Type implementation.
 * If you have more than one file using the Tesla header, only define this in the main one.
 */
...
class FullOverlay : public tsl::Gui {
   ...
   public:
    ...
    virtual void update() override {
        ...
        if (idletick0 > systemtickfrequency)
            idletick0 = systemtickfrequency;
        if (idletick1 > systemtickfrequency)
            idletick1 = systemtickfrequency;
        if (idletick2 > systemtickfrequency)
            idletick2 = systemtickfrequency;
        if (idletick3 > systemtickfrequency)
            idletick3 = systemtickfrequency;
